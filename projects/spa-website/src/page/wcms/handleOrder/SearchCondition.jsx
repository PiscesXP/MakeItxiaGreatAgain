 ...
 * 筛选条件.
 * */
function SearchConditionForm({ form: { getFieldDecorator } }) {
  return (
    <Card title="筛选">
...
const SearchCondition = Form.create({
  ...
  mapPropsToFields: ({
    condition: { onlyMine, campus, status, text, orderTime },
  }) => {
    return {
      onlyMine: Form.createFormField({ value: onlyMine }),
      campus: Form.createFormField({ value: campus }),
      status: Form.createFormField({ value: status }),
      text: Form.createFormField({ value: text }),
      orderTime: Form.createFormField({ value: orderTime }),
    };
  },
})(SearchConditionForm);
